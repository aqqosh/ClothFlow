...
def load_images(path_to_sat, path_to_tar, size=(256, 256)):
    src_list, tar_list = list(), list()
    for filename in listdir(path_to_sat)[2500:]:
        pixels = load_img(path_to_sat + filename, target_size=size)
        sat_img = img_to_array(pixels)
        src_list.append(sat_img)
    for filename in listdir(path_to_tar)[2500:]:
        pixels = load_img(path_to_tar + filename, target_size=size)
        map_img = img_to_array(pixels)
        tar_list.append(map_img)
    return [asarray(src_list), asarray(tar_list)]


# dataset path
path_to_sat = 'reDress/pairs/source/'
path_to_tar = 'reDress/pairs/redressed/'
# load dataset
[src_images, tar_images] = load_images(path_to_sat, path_to_tar)
print('Loaded: ', src_images.shape, tar_images.shape)
# save as compressed numpy array
filename = 'maps_1500_bw.npz'
...
def summarize_performance(step, g_model, dataset, n_samples=3):
	...
	print("Example has been updated")
	pyplot.close()
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=1):
	...
	for i in range(n_steps):
		...
		if i % 100 == 0:
			g_model.save('model_v1')
			print('Model saved')
 
# load image data
dataset = load_real_samples('maps_1500_bw.npz')
...
#source = cv2.imread('/home/arcsinx/ClothFlow/mask.jpg', 1)
